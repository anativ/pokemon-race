...
let acc = 0;
for (const ms of [120, 130, 150, 200]) {
  await page.evaluate(m => window.__pkr.step(m), ms);
  await page.waitForTimeout(120);
  acc += ms;
  await page.screenshot({ path: `gauntlet/shots/ri-r4-beam-t${acc}.png`, clip: { x: 420, y: 330, width: 1000, height: 560 } });
  console.log('shot t=' + acc);
}

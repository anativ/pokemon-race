...
await page.goto(`${url}/index.html?screen=race&track=pallet-town&racer=pikachu&item=hyper-beam&pos=4`);
...
for (const ms of [80, 120, 120, 120, 120]) {
  ...
  await page.screenshot({ path: `gauntlet/shots/beam-p4r2-vic-${acc}.png` });
  console.log(acc, b ? `mode=${b.mode} cone=${JSON.stringify(b.cone)}` : 'NOBEAM');
}

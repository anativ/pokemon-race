...
console.log(JSON.stringify(await page.evaluate(() => {
  ...
  return { plane: p.lane, list: s.racers.filter(r=>r.id!==p.id).map(r=>({id:r.id, gap:+(r.dist-p.dist).toFixed(1), off:+Math.abs(r.lane-p.lane).toFixed(2), sp:+r.speed.toFixed(0)})).filter(r=>r.gap>60&&r.gap<300) };
})));
